...
import io.kapaseker.kharcho.nodes.Document.OutputSettings;
...
class Printer implements NodeVisitor {
    ...
    static Printer printerFor(Node root, QuietAppendable accum) {
        ...
        if (settings.outline())     return new Outline(root, accum, settings);
        if (settings.prettyPrint()) return new Pretty(root, accum, settings);
